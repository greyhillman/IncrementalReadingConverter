    ...
    fn convert_listitem(depth: usize, item: ListItem) -> String {
        match item {
            ...
            ListItem::Nested(text, list) => {
                format!("{}\n{}", convert_textblock(text), convert_list(depth + 1, list))
            }
        }
    }
...
fn convert_table(mut table: Table) -> String {
    ...
    let body = table.body().into_iter()
        .map(convert_row);
    let body = join(body, "\n");

    match (header, footer) {
        (Some(h), Some(f)) => {
            format!("{}\n-----\n{}\n-----\n{}", convert_row(h), body, convert_row(f))
        }
        (None, Some(f)) => format!("{}\n-----\n{}", body, convert_row(f)),
        (Some(h), None) => format!("{}\n-----\n{}", convert_row(h), body),
...
mod tests {
    ...
    fn ordered_list_multiple_item() {
        let text1 = TextBlock::new()
            .add(Text::text("a"));
        let text2 = TextBlock::new()
            .add(Text::text("a"));
    ...
    fn ordered_list_nested_ordered_list() {
        let text = TextBlock::new()
            .add(Text::text("child"));
        let item = ListItem::item(text);
        let nested_list = List::new()
            .ordered()
            .add(item);

        let text = TextBlock::new()
            .add(Text::text("parent"));
    ...
    #[test]
    fn table() {
        let cell1 = TableCell::new(TextBlock::new()
                                   .add(Text::text("a")));
        let cell2 = TableCell::new(TextBlock::new()
                                   .add(Text::text("b")));
